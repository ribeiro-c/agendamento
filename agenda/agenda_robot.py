...
def extrair_eventos(login, senha):
    ...
    try:
        with sync_playwright() as p:
            ...
            # IR PARA AGENDA
            # ----------------------------
            logger.info("📅 Navegando para agenda...")
            ...
            logger.info("⏳ Aguardando carregamento do calendário...")
            page.wait_for_selector(".calendario-table-days", state="visible", timeout=30000)
            
            logger.info("📅 Calendário carregado, iniciando leitura de eventos")
            ...
            for semana_idx, semana in enumerate(semanas):
                ...
                for dia in dias:
                    try:
                        ...
                        logger.info(f"📆 Dia {numero_dia}/{data.month} tem {len(eventos)} evento(s)")
                        ...
                        for i in range(total):
                            try:
                                ...
                                logger.info(f"📌 Evento capturado: {evento_dados['titulo'][:50]}...")
                                ...
                                logger.error(f"❌ Erro ao processar evento {i + 1} do dia {numero_dia}: {e}")
                        ...
                        logger.error(f"❌ Erro ao processar dia: {e}")
                        continue
        ...
        logger.error(f"❌ Erro crítico no robô: {e}")
        traceback.print_exc()
...
if __name__ == "__main__":
    ...
    # Mostrar resumo
    print(f"\n📋 Resumo final: {len(eventos)} eventos encontrados")
